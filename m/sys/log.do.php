...
if ($op == "rollback") {
	$line = $db->query("select * from `$table` where id='$id' limit 1", 1);
	$tmp = unserialize($line["data"]);
	$t = str_replace("(", "", str_replace(")", "", $line["table_name"]));

	$ok_count = $error_count = 0;
	foreach ($tmp as $r) {
		if (count($r) > 0) {
			$sql_data = $db->sqljoin($r);
			$old_show_error = $db->show_error;
			$db->show_error = 1; //打开错误显示
			$ins_id = $db->query("insert into $t set $sql_data");
			$db->show_error = $old_show_error;
			if ($ins_id > 0) {
				$ok_count++;
			} else {
				$error_count++;
	...
	if ($ok_count > 0 && $error_count == 0) {
		$db->query("delete from `$table` where id='$id' limit 1", 1);
		echo "<br>全部恢复成功！";
	} else {
		echo "<br><br>恢复过程出现错误，请检查原因...";
	}
	exit;
}
