...
$list_heads = array(
	...
	"名称" => array("width"=>"11%", "align"=>"center"),
	"当前使用者" => array("width"=>"", "align"=>"left"),
	"时间" => array("width"=>"15%", "align"=>"center"),
...
function show_data($t, $li) {
	switch ($t) {
		...
		case "名称":
			return $li["name"];
		case "当前使用者":
			if (count($GLOBALS["ch_info"][$li["id"]]) > 30) {
				$arr = array_slice($GLOBALS["ch_info"][$li["id"]], 0, 30);
				return implode(" <font color=silver>|</font> ", $arr)." …… <font color=red>(共 <b>".count($GLOBALS["ch_info"][$li["id"]])."</b> 人)</font>";
			} else {
				return @implode(" <font color=silver>|</font> ", $GLOBALS["ch_info"][$li["id"]]);
			}
		case "时间":
		...
		case "操作":
			...
			if ($li["power_compare"] < 0) {
				if (check_power("edit")) {
					$op[] = "<a href='#edit' onclick='edit(".$li["id"].", this)' class='op' title='修改内容'>修改</a>";
				}
...
// 对排序的处理：
$sqlsort = "order by addtime asc";


$my_power = $power->get_user_power($username, $uinfo);

// 数据筛选:
$all_list = $db->query("select * from $table t $sqlwhere $sqlsort");
$use_list = array();
