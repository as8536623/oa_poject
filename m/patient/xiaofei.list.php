...
$sort_type_arr = array(
	...
	"doctor" => array("医生", "doctor {ascdesc}"),
...
$list_heads = array(
	...
	"　备注|回访" => array("align"=>"left", "width"=>"270"),
	"来院就诊时间" => array("width"=>"", "align"=>"center"),
	"疾病类型" => array("align"=>"center","width"=>""),
	"消费总额" => array("align"=>"center"),
	"消费记录" => array("align"=>"left","width"=>"200"),
	"下次回访时间" => array("width"=>"", "align"=>"center"),
...
// 默认排序方式:
	$default_sort = "order_date"; //客服或管理员则关注今天新增加了多少病人
	$default_order = "desc";
...
/*
// 2012-12-01 杨佳斌：全部未用，取消回访组长的功能，后期需要再调整
// 是回访客服组员
$is_huifang_zuyuan = 0;
...
//判断回访时间
	if($hf_time){
		...
		//if($uinfo["character_id"] == 74 || $username == "admin" || $uinfo["character_id"] == 73){
			$where[] = 	"xf_huifang_nexttime = '$hf_time'";
		/*}
		else{
			$where[] = 	"doctor = '$realname' and xf_huifang_nexttime = '$hf_time'";
		}*/
	}
//判断医生姓名
if ($doctor_name != '') {
...
$today_report = "<a>总共: <b>".$count_today_all."</b></a> &nbsp; <a href='?show=today&come=1'>已到: <b>".$count_today_come."</b></a>";
...
foreach ($list_data as $li) {
	...
	if ($id == 0) {
		...
		*/
		...
		//$r["职业"] = $li["vocation"];
		if (($show_tel || $li["author"] == $realname) && $uinfo["part_id"] != 4) {
		...
		$r["门诊号"] = '<nobr>'.$li["zhuanjia_num"].'</nobr>';
		$r["医生"] = '<nobr>'.$li["doctor"].'</nobr>';
		$dis_text = array();
		...
		if ($li["content"]) {
			$content_arr[] = '<div class="ct_1"><div class="ct_td_a c_1"><nobr>内容</nobr></div><div class="ct_td_b c_1">'.text_show($li["content"]).'</div><div class="clear"></div></div>';
		}
		if ($li["xf_memo"]) {
			$li["xf_memo"] = strip_tags(trim($li["xf_memo"]));
			if (substr_count(trim($li["xf_memo"]), "\n") >= 4) {
				$_arr = @explode("\n", str_replace("\n\n", "\n", str_replace("\r", "", trim($li["memo"]))));
			} else {
				$s_memo = text_show($li["xf_memo"]);
			}
		...
		if ($li["xf_huifang"]) {
			$li["xf_huifang"] = strip_tags(trim($li["xf_huifang"]));
			if (substr_count($li["xf_huifang"], "\n") >= 4) {
				$_arr = @explode("\n", str_replace("\n\n", "\n", str_replace("\r", "", trim($li["huifang"]))));
				$_arr = @array_slice($_arr, -4);
				$s_huifang = '<font color="gray" style="cursor:help;" title="要查看更多，请点击后面的回访图标">……(已隐藏部分较早的回访记录)……</font>'.text_show("\r\n".implode("\r\n", $_arr));
			} else {
				$s_huifang = text_show($li["xf_huifang"]);
			}
		...
		$r["　备注|回访"] = '<div class="ct_0">'.implode('<div class="hr_line"></div>', $content_arr).'</div>';

		$r["来院就诊时间"] = "<nobr>".str_replace('|', '<br>', @date((date("Y") == date("Y", $li["order_date"]) ? "Y-m-d|G:i" : "Y-m-d|G:i"), $li["order_date"]))."</nobr>";
		$xf_huifang_time = $li["xf_huifang_nexttime"] ? int_date_to_date($li["xf_huifang_nexttime"]) : "";
		$r["下次回访时间"] = "<nobr>".$xf_huifang_time."</nobr>";
		...
		$r["媒体"] = $li["media_from"];
		$r["消费总额"] = "<nobr>".$li["xiaofei_count"]."</nobr>";
		$r["消费记录"] =  text_show($li["xiaofei_log"]);
...
var base_url = "/m/patient/xiaofei.php";
...
<div id="color_tips">
颜色标记：
<?php foreach ($xf_color_tip as $k => $v) { ?>
...
<?php } ?>
	...
	<span style="margin-left:30px">
		...
		<a href="#" onClick="page_param_update('show','');page_param_update('come','', 1)">[不限]</a>&nbsp;
	</span>
