...
if ($_GET["do"] == "all") {
	$kefu_list = $db->query("select id,realname,part_id from sys_admin where hospitals='$hid' and part_id in (2,3,4)");
...
$kefu_list = $db->query("select id,realname,part_id from sys_admin where concat(',',hospitals,',') like '%,{$hid},%' and part_id in (1,2,3,4)");
