...
$aTdFormat = array(
	...
	1=>array("title"=>"疾病名称", "width"=>"15%", "align"=>"center", "sort"=>"name", "defaultorder"=>1),
	2=>array("title"=>"二级疾病", "width"=>"", "align"=>"left", "sort"=>"diaease_2", "defaultorder"=>1),
...
if ($searchword) {
	$where[] = "(binary t.name like '%{$searchword}%' or diaease_2 like '%{$searchword}%')";
}
...
var base_src = "m/patient/disease.php";

function add(hid) {
	set_high_light('');
	parent.load_src(1, base_src+'?op=add', 750, 450);
	return false;
}

function edit(id, obj) {
	set_high_light(obj);
	parent.load_src(1, base_src+'?op=edit&id='+id, 750, 450);
	return false;
}
</script>
</head>
...
<!-- 头部 begin -->
<div class="headers">
	<div class="headers_title"><table class="bar"><tr><td class="bar_left"></td><td class="bar_center"><?php echo $hospital_id_name[$hid]; ?> - 疾病列表</td><td class="bar_right"></td></tr></table></div>
	<div class="header_center">
<?php if (check_power("add")) { ?>
		<button onclick="add()" class="button">添加</button>
<?php } ?>
		&nbsp;&nbsp;&nbsp;&nbsp;
		<button onclick="location='?op=update_sort2'" class="buttonb">更新引用数</button>
	</div>
	<div class="headers_oprate"><form name="topform" method="GET">模糊搜索：<input name="searchword" value="<?php echo $_GET["searchword"]; ?>" class="input" size="12">&nbsp;<input type="submit" class="search" value="搜索" style="font-weight:bold" title="点击搜索">&nbsp;<button onclick="location='?'" class="search" title="退出条件查询">重置</button></form></div>
</div>
