...
if (count($user_field_arr) > 0) {
	...
	if ($show_xiaofei_count) {
		...
		$aTdFormat['ex_zxcb'] = array("title"=>"咨询成本", "width"=>"", "align"=>"center");
		$aTdFormat['ex_yycb'] = array("title"=>"预约成本", "width"=>"", "align"=>"center");
		$aTdFormat['ex_dzcb'] = array("title"=>"到诊成本", "width"=>"", "align"=>"center");
	}
...
<? } ?>
	...
	<!-- 主要列表数据 begin -->
<?php
$xiaofei_count = 0;$zf_count = 0;
if (count($data) > 0) {
	foreach ($data as $line) {
		$id = $line["id"];
		$xiaofei_count += floatval($line["xiaofei"]);
		$zf_count += floatval($line["x84"]);
		$line["ex_baidu"] = round($line["x5"] + $line["x6"] + $line["x7"], 1);
		$line["ex_zxcb"] = round($line["xiaofei"] / ($line["x81"] + $line["x82"]), 1);
		$line["ex_yycb"] = round($line["xiaofei"] / $line["x83"], 1);
		$line["ex_dzcb"] = round($line["xiaofei"] / $line["x84"], 1);
		if ($id == 0) {
?>
	<tr>
...
<?php if ($show_xiaofei_count) { ?>
	&nbsp;本页总计消费额(<b><?php echo $xiaofei_count; ?></b>) / 天数(<b><?php echo count($data); ?></b>) = 日均消费(<b><?php echo @round($xiaofei_count / count($data), 1); ?></b>),本月已到诊(<b><?php echo $zf_count; ?></b>)，到诊成本为（<?php echo @round($xiaofei_count / $zf_count,1); ?>）元。<?php } ?>
	</div>
