...
picker; (function() {
	...
	function J() {
		...
		for (var B = 0; B < $.length; B++) {
			_ = $[B].src.substring(0, $[B].src.toLowerCase().indexOf("picker.js"));
			A = _.lastIndexOf("/");
	...
	function I(G, A) {
		...
		if (!$dp.dd || $dp.eCont || ($dp.lang && $dp.realLang && $dp.realLang.name != $dp.lang)) {
			...
			if ($.$dpPath == "") D(_);
			var B = "<iframe src=\"" + $.$dpPath + "picker.htm\" frameborder=\"0\" border=\"0\" scrolling=\"no\"></iframe>";
			if ($dp.eCont) {
		...
		function C() {
			...
			$dp.dd.style.top = B + 1 + "px";
			$dp.dd.style.left = F + 2 + "px"
		}
	}
})()