/*
	SortTable modify by zhuwenya

	* 用法：将要排序的表格添加CSS，如<table class="sortable">
	  对于不需要排序的列，在表头所在行的该列，添加CSS，如<td class="sorttable_nosort"></td>
	* 新增功能：当点击排序后，刷新页面后仍然保持该排序 cookie记录
	  注意需要 md5.js 支持 (计算页面名称的md5值等)
*/

document.write('<scr'+'ipt src="/res/md5.js" language="javascript"></scr'+'ipt>');
	...
	init: function() {
		...
		forEach(document.getElementsByTagName('table'), function(table) {
			if (table.className.search(/\bsortable\b/) != -1) {
				...
				// 保持排序 zhuwenya @ 2013-01-29
				var table_id = hex_md5(wee_get_page_name())+"_"+table.id;
				var last_sort = get_cookie(table_id);
				if (last_sort) {
					var tmp = last_sort.split("@");
					var sort_field = tmp[0];
					var sort_asc_desc = tmp[1];

					var headrow = table.tHead.rows[0].cells;
					for (var i=0; i<headrow.length; i++) {
						if (sort_field == wee_get_head_name(headrow[i])) {
							headrow[i].click();
		...
		for (var i=0; i<headrow.length; i++) {
			...
			if (!headrow[i].className.match(/\bsorttable_nosort\b/)) { // skip this col
				...
				dean_addEvent(headrow[i], "click", function(e) {
					...
					var table_id = hex_md5(wee_get_page_name())+"_"+this.parentNode.parentNode.parentNode.id;
					//alert(table_id);
					var sort_field = wee_get_head_name(this);
					...
					if (this.className.search(/\bsorttable_sorted\b/) != -1) {
						...
						// zhuwenya @ 2013-01-29
						sort_asc_desc = "asc";
						wee_save_sort(table_id, sort_field, sort_asc_desc);
						wee_set_sort_flag(table, sort_field, sort_asc_desc);
...
// 获取表头的名称 要去除排序标记
function wee_get_head_name(o) {
	return o.innerHTML.split("↑")[0].split("↓")[0];
}

// 设置排序标记 by zhuwenya @ 2013-01-31
function wee_set_sort_flag(table, sort_field, sort_asc_desc) {
	var headrow = table.tHead.rows[0].cells;
	for (var i=0; i<headrow.length; i++) {
		var hn = wee_get_head_name(headrow[i]);
		if (sort_field == hn) {
			headrow[i].innerHTML = hn + (sort_asc_desc == "asc" ? "↑" : "↓");
		} else {
			headrow[i].innerHTML = hn;
		}
	}
	return true;
}
...
// 获取页面名称，用于cookie保存用
function wee_get_page_name() {
	return self.location.href.split("#")[0].split("?")[0];
}

// 将当前的排序方法保存到cookie，用于刷新页面后重新执行
function wee_save_sort(table_id, sort_head, sort_type) {
	set_cookie(table_id, sort_head+"@"+sort_type, 10000000);

	// 将排序方式发送回服务器:
	//var js = "/sort_count/count.php?sort_head="+encodeURIComponent(sort_head)+"&sort_type="+sort_type+"&t="+Math.random();
	//wee_load_js(js);
}
