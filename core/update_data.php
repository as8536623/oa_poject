...
if (time() - $last_update > $update_interval) {
	...
	foreach ($_hlist as $_id => $_name) {
		...
		$qz = mysql_query("SELECT hid,u_name,addtime FROM ku_list WHERE (addtime>=$tb and addtime<=$te and hid=$_id)", $db->dblink);
		$dataz = array();
		while ($liz = mysql_fetch_assoc($qz)) {
			$dataz[] = $liz;
		}
		...
		foreach ($data as $li) {
			...
			foreach ($time_arr as $tn => $d) {
				// 总
				if ($ad >= $d[0] && $ad <= $d[1]) {
					if ($li["status"] == 0||$li["status"] == 1||$li["status"] == 2)$res["总"]["预约"][$tn] += 1;if ($li["status"] == 3) $res["总"]["跟踪"][$tn] += 1;
				}
				if ($od >= $d[0] && $od <= $d[1]) {
					if ($li["status"] == 0||$li["status"] == 1||$li["status"] == 2)$res["总"]["预到"][$tn] += 1;
					if ($li["status"] == 1) $res["总"]["实到"][$tn] += 1;
				}

				// 网络
				if ($li["part_id"] == 2 && $ad >= $d[0] && $ad <= $d[1]) {
					if ($li["status"] == 0||$li["status"] == 1||$li["status"] == 2)$res["网络"]["预约"][$tn] += 1;
				}
				if ($li["part_id"] == 2 && $od >= $d[0] && $od <= $d[1]) {
					if ($li["status"] == 0||$li["status"] == 1||$li["status"] == 2)$res["网络"]["预到"][$tn] += 1;
					if ($li["status"] == 1) $res["网络"]["实到"][$tn] += 1;
				}

				// 电话
				if ($li["part_id"] == 3 && $ad >= $d[0] && $ad <= $d[1]) {
					if ($li["status"] == 0||$li["status"] == 1||$li["status"] == 2)$res["电话"]["预约"][$tn] += 1;
				}
				if ($li["part_id"] == 3 && $od >= $d[0] && $od <= $d[1]) {
					if ($li["status"] == 0||$li["status"] == 1||$li["status"] == 2)$res["电话"]["预到"][$tn] += 1;
		...
		//咨询数据
		foreach ($dataz as $liz) {
			$adz = $liz["addtime"]; //ad为addtime
			foreach ($time_arr as $tnz => $dz) {
				// 总
				if ($adz >= $dz[0] && $adz <= $dz[1]) {
					$res["总"]["咨询"][$tnz] += 1;
				}


				// 个人
				$liz["u_name"] = trim($liz["u_name"]);
				if ($liz["u_name"] != '' && $adz >= $dz[0] && $adz <= $dz[1]) {
					$res["个人_".$liz["u_name"]]["咨询"][$tnz] += 1;
				}
