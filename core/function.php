...
function patient_modify_log($new_arr, $old_arr, $log_field='') {
	...
	foreach ($log_fields as $f) {
		if (!empty($f) && @array_key_exists($f, $new_arr)) {
			if ((!empty($new_arr[$f]) || !empty($old_arr[$f])) && $new_arr[$f] != $old_arr[$f]) { //新数据或老数据至少有一个不为空
				...
				$log_item[] = noe($field_to_name[$f], $f)."由【".$str_old."】修改为【".$str_new."】";
			}
		}
	}
	...
	if (count($log_item) > 0) {
		global $realname;
		return trim($old_arr["edit_log"]."\r\n".date("Y-m-d H:i:s")." ".$realname." 将".implode("、", $log_item)."。");
	}
...
function patient_modify_log_s($new_arr, $old_arr, $log_field='') {
	...
	if (count($log_item) > 0) {
		global $realname;
		return trim($realname." 将".implode("、", $log_item)."。");
	}
...
function pagelinkc($page, $pagecount, $reccount='-1', $linkbase='') {
	...
	$pagelink .= '<button onclick="location='."'".$base.'page='.($page-1)."'".'"'.$useful.' class="button">上页</button>'.$sp;
	$useful = $page < $pagecount ? '' : 'disabled="true"';
	$pagelink .= '<button onclick="location='."'".$base.'page='.($page+1)."'".'"'.$useful.' class="button">下页</button>'.$sp;
	...
	if ($pagecount > $bigpage) {
		...
		$pagelink .= '<button onclick="location='."'".$base.'page='."'"."+document.getElementById('pltext.').value;".'"'.' class="button">确定</button>';
	}
	$pagelink .= '</div>';
...
function exit_html($str, $color='') {
	echo '<div style="font-family:微软雅黑,Tahoma; font-size:12px; color:'.$color.';">'.$str.'</div>';
	exit;
}
...
function put_talk_content($hid, $id, $content) {
	...
	if (trim($content) == '') {
		if (@file_exists($dir.$id.".txt")) {
			@unlink($dir.$id.".txt"); //如果没有内容，则不保存，并删除原先的文件
		}
		return 0;
	} else {
		// 建立医院目录:
		...
		@chmod($hdir, 0777);
		if (!file_exists($hdir)) {
			exit($hdir." 建立不成功，请立即联系开发人员检查目录权限");
		}
		...
		@chmod($dir, 0777);
		if (!file_exists($dir)) {
			exit($dir." 建立不成功，请立即联系开发人员检查目录权限");
		}
