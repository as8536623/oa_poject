...
class power {
	...
	*/
	...
	function show_power_table($_menu, $cur_menu, $show_check_all=1) {
		...
		if ($show_check_all) {
			$out .= '<input type="checkbox" checked id="check_op" style="display:none;"><input type="button" onclick="set_check({check_all}, byid(\'check_op\'))" value="全部选中" class="buttonb">&nbsp;<input type="button" onclick="set_check({check_all}, this)" value="全部不选" class="buttonb">&nbsp;<input type="button" onclick="this.form.reset()" value="还原为默认" class="buttonb">';
		}

		$out .= '<table width="100%" class="power_1" cellspacing="0" cellpadding="0" style="margin-top:10px;">';
		...
		foreach ($m_stru as $p1 => $v2) {
			...
			$out .= '<td class="right">';
			if ($v2) {
				$out .= '<table width="100%" class="power_2" cellspacing="0" cellpadding="0">';
				foreach ($v2 as $p2) {
					$_cid = "item_".$p2;
	...
	function compare_hospitals($user_line, $my_line) {
		...
		if ($hids == $my_hospital_ids) return 0;
